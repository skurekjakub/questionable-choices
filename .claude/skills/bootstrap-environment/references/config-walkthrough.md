...
`code` on WSL's PATH is usually the VS Code _server_ shim, which only works
...
| `defaultPermissionMode` | default · acceptEdits · auto · bypassPermissions · manual · dontAsk · plan | the example defaults to `bypassPermissions`: sessions run unattended |
...
- `emailEnv` / `tokenEnv`: variable _names_. Confirm they are exported in the
